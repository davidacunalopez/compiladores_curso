...
class Parser:
    def __init__(self):
        ...
        self.error = False
        self.arbol = []  # Lista para almacenar el árbol sintáctico
        self.nivel_actual = 0  # Nivel actual en el árbol
        self.terminales = {
    ...
    def agregar_nodo(self, simbolo, lexema=None):
        """Agrega un nodo al árbol sintáctico"""
        indentacion = "  " * self.nivel_actual
        if lexema:
            self.arbol.append(f"{indentacion}{simbolo}: {lexema}")
        else:
            self.arbol.append(f"{indentacion}{simbolo}")

    def aplicar_regla(self, numero_regla):
        """Aplica una regla de la gramática"""
        if numero_regla in self.tabla_lados_derechos:
            # Obtener el no-terminal actual
            no_terminal_actual = self.pila_parsing[-1] if self.pila_parsing else None
            if no_terminal_actual:
                self.agregar_nodo(no_terminal_actual)
                self.nivel_actual += 1
    ...
    def match(self, terminal_esperado):
        ...
        if self.token_actual.codigo == 61 and terminal_esperado == 'identificador':
            ...
            self.agregar_nodo('identificador', self.token_actual.lexema)
            self.token_actual = self.scanner.DemeToken()
            return True
        ...
        # Si es un literal (código 50) y el terminal esperado es 'literal'
            ...
            self.agregar_nodo('literal', self.token_actual.lexema)
            self.token_actual = self.scanner.DemeToken()
            return True
        ...
        # Para otros casos, comparar el lexema
            ...
            self.agregar_nodo(terminal_esperado)
            self.token_actual = self.scanner.DemeToken()
            return True
...
if __name__ == "__main__":
    parser = Parser()
    if parser.inicializar_parser("Entregable 1/Entregables/Scanner/prueba.ne"):
        parser.parse()
